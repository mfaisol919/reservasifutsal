<script lang="ts">
  import { BACKEND_URL, auth } from '$lib/auth.svelte.js';
...
<svelte:head>
  <title>Login - Arena Futsal Hub</title>
</svelte:head>
...
<div class="min-h-[80vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-950 relative overflow-hidden">
  ...
  <div class="absolute w-[400px] h-[400px] bg-emerald-500/10 rounded-full blur-3xl -top-20 -left-20"></div>
  <div class="absolute w-[400px] h-[400px] bg-teal-500/10 rounded-full blur-3xl -bottom-20 -right-20"></div>
  ...
  <div class="max-w-md w-full space-y-8 relative z-10">
    <div class="text-center">
      ...
      <p class="mt-2 text-sm text-slate-400">
        Masuk untuk memesan lapangan futsal Anda secara instan.
      </p>
    </div>
        ...
        >
          ...
          {:else}
            <span class="flex items-center gap-1.5">
              <LogIn class="w-4 h-4" /> Masuk Akun
            </span>
          {/if}
        </button>
      </form>
